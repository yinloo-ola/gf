...
// DriverBbsql is the driver for mysql database.
type DriverBbsql struct {
	*Core
}

// New creates and returns a database object for mysql.
// It implements the interface of gdb.Driver for extra database driver installation.
func (d *DriverBbsql) New(core *Core, node *ConfigNode) (DB, error) {
	return &DriverBbsql{
		Core: core,
	}, nil
}
...
func (d *DriverBbsql) Open(config *ConfigNode) (*sql.DB, error) {
	var source string
	if config.LinkInfo != "" {
	...
	if db, err := sql.Open("bbsql", source); err == nil {
		return db, nil
	} else {
		return nil, err
	}
...
func (d *DriverBbsql) FilteredLinkInfo() string {
...
func (d *DriverBbsql) GetChars() (charLeft string, charRight string) {
	return "`", "`"
}

// HandleSqlBeforeCommit handles the sql before posts it to database.
func (d *DriverBbsql) HandleSqlBeforeCommit(link Link, sql string, args []interface{}) (string, []interface{}) {
	return sql, args
}

// Tables retrieves and returns the tables of current schema.
// It's mainly used in cli tool chain for automatically generating the models.
func (d *DriverBbsql) Tables(schema ...string) (tables []string, err error) {
	var result Result
...
// process restarts.
func (d *DriverBbsql) TableFields(table string, schema ...string) (fields map[string]*TableField, err error) {
